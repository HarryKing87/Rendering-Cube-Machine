...
const Vector3D Vertex::VectorFromVertex(const Vertex& rhs) const
{
...
Vector3D Vertex::MinusToVector(const Vertex& other) const
{
	Vector3D temp;
	temp.SetX(_x - other.GetX());
	temp.SetY(_y - other.GetY());
	temp.SetZ(_z - other.GetZ());
	return temp;
}